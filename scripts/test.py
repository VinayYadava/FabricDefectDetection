...
def get_results( cfg):
    ...
    for img_name in file_list:
        ...
        print(f"fcgfc {img_path}")
        img_name = img_name[:-4]
        print(f"bokachoda {img_name}")
        c = 'i' if not cfg.sub_folder else cfg.sub_folder
        ...
        print(cfg.save_dir+'/'+img_name+'_residual.png')
        cv2.imwrite(cfg.save_dir+'/'+img_name+'_residual.png', mask)
        cv2.imwrite(cfg.save_dir+'/'+img_name+'_origin.png', test_img)
        cv2.imwrite(cfg.save_dir+'/'+img_name+'_rec.png', rec_img)
        cv2.imwrite(cfg.save_dir+'/'+img_name+'_visual.png', vis_img)
        print("madarchod")
